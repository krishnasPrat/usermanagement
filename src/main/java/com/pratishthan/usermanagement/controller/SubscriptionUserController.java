...
public class SubscriptionUserController {
    ...
    @PostMapping("/subscription/{subscriptionId}/user/{userId}/special-permissions")
    public ResponseEntity<SubscriptionUserDTO> upsertSpecialPermissions(
            @PathVariable Long subscriptionId,
            @PathVariable Long userId,
            @RequestBody List<SpecialPermissionDTO> permissions
    ) {
        return ResponseEntity.ok(subscriptionUserService.upsertSpecialPermissions(subscriptionId, userId, permissions));
    }
}
